...
int main(int argc, char **argv) {
    ...
    QTimer::singleShot(3000, &app, [&] {
        ...
        const bool gotPeaks = rootFiles > 0 && peaksBuilder.peaks(fileModel.filePath(0)).valid();

        const QModelIndex root    = folderModel.index(0, 0);
        const int       subdirs   = folderModel.rowCount(root);
        int             nestedRows = 0;
        if (subdirs > 0) {
            window.selectDirectory(folderModel.index(0, 0, root));
            nestedRows = fileModel.rowCount();
        }

        std::printf("rootFiles=%d peaks=%s subdirs=%d nestedFiles=%d\n", rootFiles, gotPeaks ? "yes" : "no",
                    subdirs, nestedRows);
        exitCode = (rootFiles >= 1 && gotPeaks && subdirs >= 0) ? 0 : 1;
        app.quit();
    });
