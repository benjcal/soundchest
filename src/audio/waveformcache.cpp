...
public:
    ...
    void run() override
    {
        ...
        QMetaObject::invokeMethod(
            m_cache,
            [cache = m_cache, filePath = m_filePath, data = std::move(data),
             generation = m_generation]() mutable {
...
WaveformData WaveformCache::get(const QString& filePath) const
{
    return m_cache.value(filePath);
}

bool WaveformCache::isCached(const QString& filePath) const
{
